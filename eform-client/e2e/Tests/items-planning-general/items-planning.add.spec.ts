...
describe('Items planning - Add', function () {
  ...
  it('check all fields planning', function () {
    ...
    expect(planningRowObject.name, 'Saved name in table is incorrect').equal(planningData.name[1]);
    expect(planningRowObject.eFormName, 'Saved template in table is incorrect').equal(planningData.eFormName);
    expect(planningRowObject.description, 'Saved description in table is incorrect').equal(planningData.description);
    expect(planningRowObject.repeatEvery, 'Saved repeat every in table is incorrect').equal(planningData.repeatEvery);
    const repeatUntil = new Date(planningData.repeatUntil);
    expect(planningRowObject.repeatUntil.getDate(), 'Saved repeat Until in table is incorrect').equal(repeatUntil.getDate());
    expect(planningRowObject.repeatType, 'Saved repeat type in table is incorrect').equal(planningData.repeatType);
